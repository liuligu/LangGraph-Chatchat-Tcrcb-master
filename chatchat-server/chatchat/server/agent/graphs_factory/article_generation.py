...
from .graphs_registry import register_graph, Graph
...
@register_graph
class ArticleGenerationGraph(Graph):
    ...
    # 用来爬取用户输入的网页文章内容
    def get_spider_graph(self, memory: Any) -> CompiledStateGraph:
        ...
        llm_with_structured_output = respond_template | self.llm.with_structured_output(ArticleListResponse)
    ...
    def get_graph(self) -> CompiledGraph:
        ...
        spider_graph = self.get_spider_graph(memory=self.checkpoint)

        graph_builder.add_node("article_generation_init_break_point", self.article_generation_init_break_point)
        graph_builder.add_node("url_handler", self.url_handler)
        graph_builder.add_node("spider", spider_graph)
        graph_builder.add_node("article_generation_start_break_point", self.article_generation_start_break_point)
        graph_builder.add_node("writer", self.article_writer)
        graph_builder.add_node("article_generation_repeat_break_point", self.article_generation_repeat_break_point)
        graph_builder.add_node("rewriter", self.article_rewriter)
        ...
        graph = graph_builder.compile(checkpointer=self.checkpoint, interrupt_after=[
