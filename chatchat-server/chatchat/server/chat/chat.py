...
def create_models_from_config(configs, callbacks, stream, max_tokens):
    ...
    for model_type, params in configs.items():
        ...
        print(f"000000000000000000000000000000000{model_name}")
        models[model_type] = model_instance
        print(f"1111111111111111111111111111111{params}")
        prompt_name = params.get("prompt_name", "default")
        print(f"222222222222222222222222222222222222{prompt_name}")
        prompt_template = get_prompt_template(type=model_type, name=prompt_name)
        print(f"33333333333333333333333333333333333{prompt_template}")
        prompts[model_type] = prompt_template
    print(f"444444444444444444444444444444444444444444444444444{prompts}")
...
def create_models_chains(
    ...
    if history:
        ...
        print(f"444444444444444444444444444444444444444444444444444444444444444444444444{prompts}")
        print(f"444444444444444444444444444444444444444444444444444444444444444444444444{prompts["llm_model"]}")
        input_msg = History(role="user", content=prompts["llm_model"]).to_msg_template(
            False
        )
...
async def chat(
    ...
    async def chat_iterator() -> AsyncIterable[OpenAIChatOutput]:
        try:
            ...
            print(f"333333333333333333333333333333333333333333333333333333333333333333333333{History}")
            ...
            async for chunk in callback.aiter():
                ...
                if data["status"] in [AgentStatus.tool_end]:
                    ...
                    try:
                        ...
                        ...
                print(f"44444444444444444444444444444444444444444444444444444444{History}")
